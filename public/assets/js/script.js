        ...
        .then((poetryData) => {
            ...
            poetryData.forEach((p) => {
                ...
                $("<div>", { class: "slide" })
                    .html(
                        `
        <div class="latest"><p>${l1}</p><p>${l2}</p></div>
        <div class="poetName x2">
          <a href="${p.link || "#"}" target="_blank">${p.poet || "Unknown"}</a>
        </div>
      `
                    )
                    .appendTo($top5);
        ...
        .catch((err) => console.error("Error loading Top Shayari data:", err));
